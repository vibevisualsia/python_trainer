...
FEEDBACK_BG = "#1e1e1e"
FEEDBACK_FG = "#d4d4d4"
_bullet_setting = (os.getenv("PYTHON_TRAINER_WARNING_BULLET", "-") or "-").strip()
WARNING_BULLET = _bullet_setting if _bullet_setting else "-"
WARNING_MAX_LINES = 10
GUTTER_BG = "#1a1a1a"
GUTTER_FG = "#777777"
GUTTER_FG_ACTIVE = "#ffffff"
EDITOR_CURRENT_LINE_BG = "#303847"
...
class AppTkinter:
    def __init__(self, exam_mode: bool = False) -> None:
        ...
        self.skip_highlight_once = False
    ...
    def _build_body(self) -> None:
        ...
        self.exam_lock_var = tk.StringVar(value="")
        self.exam_lock_label = tk.Label(
            hints_frame,
            textvariable=self.exam_lock_var,
            font=("Segoe UI", 10),
            fg="#b26a00",
            anchor="w",
            justify="left",
        )
        self.exam_lock_label.grid(row=2, column=0, columnspan=7, sticky="w", pady=(4, 0))
        ...
        for button in (self.hint1_btn, self.hint2_btn, self.solution_button):
            button.bind("<Enter>", self._on_exam_disabled_feature_attempt, add="+")
            button.bind("<Button-1>", self._on_exam_disabled_feature_attempt, add="+")
        ...
        self.line_numbers = tk.Canvas(editor_frame, width=32, bg=GUTTER_BG, highlightthickness=0)
        ...
        code_scroll_x = tk.Scrollbar(editor_frame, command=self.code_text.xview, orient="horizontal")
        ...
        self.code_text.tag_configure("current_line", background=EDITOR_CURRENT_LINE_BG)
        ...
        editor_frame.bind("<Configure>", lambda event: self._update_line_numbers())
        ...
        self.live_lint_label.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(4, 0))
        ...
        feedback_frame.rowconfigure(6, weight=0)
        feedback_frame.rowconfigure(8, weight=1)
        ...
        self.warning_label = tk.Label(feedback_frame, text="AVISO", font=("Segoe UI", 10, "bold"), fg="#b26a00")
        self.warning_label.grid(row=3, column=0, sticky="w", pady=(8, 0))
        ...
        self.warning_text.grid(row=4, column=0, sticky="nsew")
        self.warning_scroll = tk.Scrollbar(feedback_frame, command=self.warning_text.yview)
        self.warning_scroll.grid(row=4, column=1, sticky="ns")
        self.warning_text.configure(yscrollcommand=self.warning_scroll.set, state="disabled")
        self.warning_label.grid_remove()
        self.warning_text.grid_remove()
        self.warning_scroll.grid_remove()

        self.hint_label = tk.Label(feedback_frame, text="PISTA", font=("Segoe UI", 10, "bold"), fg="#57a64a")
        self.hint_label.grid(row=5, column=0, sticky="w", pady=(8, 0))
        ...
        self.hint_text.grid(row=6, column=0, sticky="nsew")
        self.hint_scroll = tk.Scrollbar(feedback_frame, command=self.hint_text.yview)
        self.hint_scroll.grid(row=6, column=1, sticky="ns")
        self.hint_text.configure(yscrollcommand=self.hint_scroll.set, state="disabled")
        self.hint_label.grid_remove()
        self.hint_text.grid_remove()
        self.hint_scroll.grid_remove()

        self.stdout_label = tk.Label(feedback_frame, text="Salida (stdout/stderr)", font=FONT_SUBTITLE)
        self.stdout_label.grid(row=7, column=0, sticky="w", pady=(8, 0))
        ...
        self.stdout_text.grid(row=8, column=0, sticky="nsew")
        self.stdout_scroll = tk.Scrollbar(feedback_frame, command=self.stdout_text.yview)
        self.stdout_scroll.grid(row=8, column=1, sticky="ns")
        ...
        self.details_frame.grid(row=9, column=0, sticky="nsew", pady=(8, 0))
    ...
    def _exam_disabled_message(self) -> str:
        if self.mode == "examen":
            return "Modo Examen: Pista y Solución están deshabilitadas."
        return ""

    def _on_exam_disabled_feature_attempt(self, event=None):
        message = self._exam_disabled_message()
        self.exam_lock_var.set(message)
        if message:
            return "break"
        return None
    ...
    def _normalize_warnings(self, raw_warnings: Any) -> list[str]:
        if raw_warnings is None:
            return []

        if isinstance(raw_warnings, str):
            source_items = raw_warnings.splitlines()
        elif isinstance(raw_warnings, (list, tuple)):
            source_items = []
            for item in raw_warnings:
                if item is None:
                    continue
                source_items.extend(str(item).splitlines())
        else:
            source_items = str(raw_warnings).splitlines()
    ...
    def _set_warnings(self, warnings: list[str], exam_mode: bool) -> None:
        if len(warnings) == 0:
            self.warning_label.grid_remove()
            self.warning_text.grid_remove()
            self.warning_scroll.grid_remove()
        ...
        if exam_mode:
            warning_text = "Aviso: la ejecucion se ha realizado con limitaciones en este sistema."
        else:
            shown_warnings = warnings[:WARNING_MAX_LINES]
            warning_lines = [f"{WARNING_BULLET} {item}" for item in shown_warnings]
            remaining = len(warnings) - len(shown_warnings)
            if remaining > 0:
                noun = "aviso" if remaining == 1 else "avisos"
                warning_lines.append(f"(+{remaining} más {noun})")
            warning_text = "\n".join(warning_lines)

        self.warning_label.grid()
        self.warning_text.grid()
        self.warning_scroll.grid()
        self._set_text_widget(self.warning_text, warning_text)
        self.warning_text.yview_moveto(0)

    def _set_hint(self, hint_text: Optional[str]) -> None:
        if not hint_text:
            self.hint_label.grid_remove()
            self.hint_text.grid_remove()
            self.hint_scroll.grid_remove()
            self._set_text_widget(self.hint_text, "")
            return
        self.hint_label.grid()
        self.hint_text.grid()
        self.hint_scroll.grid()
        self._set_text_widget(self.hint_text, hint_text)
        self.hint_text.yview_moveto(0)

    def _text_for_hint_scan(self, code: str) -> str:
    ...
    def _detect_hint(self, code: str, result: Dict[str, Any], study_mode: bool = True) -> Optional[str]:
        if not study_mode:
            return None

        text = code or ""
        scan_text = AppTkinter._text_for_hint_scan(self, text)
        status = str(result.get("status", "")).lower()
        message = str(result.get("message", "")).lower()
        stderr_text = str(result.get("stderr", "")).lower()
        details_text = str(result.get("details", "")).lower()
        combined_error = f"{message}\n{stderr_text}\n{details_text}"

        is_timeout = status == "timeout" or "timeout" in combined_error or "tiempo excedido" in combined_error
        infinite_while = re.search(r"\bwhile\s+(?:True|1)\b", scan_text) and not re.search(r"\bbreak\b", scan_text)
        sleep_usage = re.search(r"\b(?:time\.)?sleep\s*\(", scan_text)
        division_zero = re.search(r"/\s*0(?![\d.])", scan_text)

        if is_timeout:
            if infinite_while:
                return "Pista: Parece que tu programa nunca termina. Revisa si tienes un while True sin break."
            if sleep_usage:
                return "Pista: Estas usando sleep(). Si es muy largo, la ejecucion puede tardar demasiado."

        if "zerodivisionerror" in combined_error or division_zero:
            return "Pista: Hay una division entre cero. Revisa el divisor antes de dividir."

        for variable in ("total", "suma", "resultado"):
            if re.search(rf"^\s*{variable}\s*=", scan_text, flags=re.MULTILINE):
                if not re.search(rf"\bprint\s*\([^)]*\b{variable}\b", scan_text):
                    return "Pista: Has calculado un valor, pero no lo estas mostrando. Prueba a imprimirlo con print()."

        for match in re.finditer(r"^\s*def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(", scan_text, flags=re.MULTILINE):
            function_name = match.group(1)
            body = scan_text[match.end():]
            if re.search(rf"\b{function_name}\s*\(", body) and not re.search(r"\bif\b", body):
                return "Pista: Parece recursion. Asegurate de tener un caso base para que termine."
    ...
    def _update_line_numbers(self) -> None:
        ...
        while True:
            ...
            gutter_width = int(self.line_numbers.cget("width"))
            current_line = self.code_text.index("insert").split(".")[0]
            color = GUTTER_FG_ACTIVE if line_num == current_line else GUTTER_FG
    ...
    def _reset_feedback_panel(self) -> None:
        ...
        self.exam_lock_var.set("")
        self._set_text_widget(self.feedback_text, "")
        self._set_warnings([], exam_mode=False)
        self._set_hint(None)
        self._set_output("", "")
    ...
    def _render_feedback(self, result: Dict[str, Any], code: str = "") -> None:
        ...
        if raw_warnings is None:
            ...
            warnings = self._normalize_warnings(raw_warnings)
        if status == "error" and details and not stderr_text:
            stderr_text = details

        self._set_status(status)
        self._clear_code_error_highlight()
        if status in ("fail", "error", "timeout", "blocked"):
        ...
        if self.mode == "examen":
            ...
            self.details_btn.grid_remove()
            self._set_hint(None)
            self._set_warnings(warnings, exam_mode=True)
        ...
        # Modo estudio
        hint_text = self._detect_hint(code, result, study_mode=True)
        self._set_hint(hint_text)
        self._set_warnings(warnings, exam_mode=False)
    ...
    def _run_worker(self, run_id: int, kind: str, code: str, exercise: Dict) -> None:
        try:
            if kind == "check":
                preflight = run_user_code(
                    code,
                    setup=exercise.get("setup", {}),
                    timeout_s=self.run_timeout_ms / 1000,
                )
                if preflight.get("status") == "ok":
                    ...
                    result = self._map_runner_result(preflight, as_check=True)
            else:
                exec_result = run_user_code(
                    code,
                    setup=exercise.get("setup", {}),
                    timeout_s=self.run_timeout_ms / 1000,
                )
                result = self._map_runner_result(exec_result, as_check=False)
    ...
    def _map_runner_result(self, runner_result: Dict, as_check: bool) -> Dict[str, Any]:
        runner_status = runner_result.get("status", "error")
        stdout_text = runner_result.get("stdout", "")
        stderr_text = runner_result.get("stderr", "")
        ...
        if runner_status == "ok":
            if as_check:
                return {
                    "status": "error",
                    "message": "Error interno: la validacion no se ejecuto.",
                    "stdout": stdout_text,
                    "stderr": stderr_text,
                    "details": stderr_text,
                    "warnings": warnings,
        ...
        if runner_status == "timeout":
            timeout_message = message or "Tiempo limite excedido."
            timeout_stderr = stderr_text if stderr_text else f"{timeout_message}\n"
            return {
                "status": "timeout",
                "message": timeout_message,
                "stdout": stdout_text,
                "stderr": timeout_stderr,
                "details": timeout_stderr,
                "warnings": warnings,
            }

        if runner_status == "blocked":
            block_message = message or "Codigo bloqueado por seguridad."
            details = stderr_text if stderr_text else block_message
            return {
                "status": "blocked",
        ...
        error_message = message or "Error en ejecucion."
        details = stderr_text if stderr_text else error_message
        return {
            "status": "error",
        ...
        if self.run_thread and self.run_thread.is_alive():
            ...
            if elapsed_ms >= self.run_timeout_ms:
                ...
                self._render_feedback(
                    {"status": "timeout", "message": msg, "stdout": "", "stderr": msg + "\n", "details": ""},
                    code=self.code_text.get("1.0", "end-1c"),
                )
                return
            self.run_timeout_after_id = self.root.after(100, lambda: self._check_run_timeout(run_id))
        ...
        try:
            while True:
                ...
                if kind == "check":
                    ...
                    self._update_solution_button()
                    self._render_feedback(result, code=code)
                else:
                    self._render_feedback(result, code=code)
